...
is_replace_cup_mode = False   # 雙擊後持續顯示更換杯子畫面，直到新杯子放上且穩定偵測啟動
...
while True:
    ...
    # --- 邏輯分支 2：水杯被拿起來 (燈光特效、顯示喝水中) ---
        ...
        if is_on_coaster:
            is_on_coaster = False
            if not is_replace_cup_mode:
                display.show_drinking()
            last_display_ticks = current_ticks
            data_changed = True

        # 更換杯子模式：持續刷新更換杯子畫面，直到新杯子放上
        if is_replace_cup_mode:
            if utime.ticks_diff(current_ticks, last_display_ticks) > DISPLAY_INTERVAL_MS:
                display.show_replace_cup()
                last_display_ticks = current_ticks
