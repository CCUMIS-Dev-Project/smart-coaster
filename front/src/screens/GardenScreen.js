...
const FLOWER_DATA = [
  { name: '向日葵', language: '崇拜與忠誠', desc: '向日葵永遠朝向太陽，象徵對光明的追求與對摯愛的忠貞不渝。' },
  { name: '玫瑰',   language: '熱烈的愛',   desc: '深紅玫瑰是愛情最古老的符號，代表深沉、熾熱且無畏的愛意。' },
  { name: '櫻花',   language: '生命之美',   desc: '短暫而燦爛的花期提醒我們珍惜當下，感受每一個稍縱即逝的美好瞬間。' },
  { name: '鬱金香', language: '完美的愛',   desc: '鬱金香高雅挺立，象徵高貴的愛情與對完美伴侶的深切渴望。' },
  { name: '薰衣草', language: '等待與寧靜', desc: '薰衣草的紫色香氣令人心靜，代表在寧靜中等待，以及對愛人的思念。' },
  { name: '雛菊',   language: '純真與希望', desc: '雛菊潔白無瑕，象徵純真的心靈與對未來充滿希望的輕盈心境。' },
  { name: '水仙',   language: '自尊與重生', desc: '水仙在嚴冬後第一個綻放，象徵自我尊重、新的開始與內心的重生。' },
  { name: '彩虹花', language: '多彩與奇蹟', desc: '如彩虹般絢爛，象徵生命中每一種色彩都是奇蹟，擁抱所有的可能性。' },
];

function Flower1({ size = 40, colored }) {
  const c = colored;
  const pc = c ? '#F5A623' : '#ccc', pc2 = c ? '#E8941A' : '#bbb';
  const cc = c ? '#4A2500' : '#aaa', cc2 = c ? '#3A1A00' : '#999';
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      {[0,20,40,60,80,100,120,140,160,180,200,220,240,260,280,300,320,340].map((a,i) => (
        <Ellipse key={i} cx="40" cy="14" rx="4" ry="10" fill={i%2===0?pc:pc2} transform={`rotate(${a} 40 40)`} />
      ))}
      {[10,30,50,70,90,110,130,150,170,190,210,230,250,270,290,310,330,350].map((a,i) => (
        <Ellipse key={`b${i}`} cx="40" cy="20" rx="3" ry="7" fill={pc2} opacity={0.7} transform={`rotate(${a} 40 40)`} />
      ))}
      <Circle cx="40" cy="40" r="16" fill={cc} />
      <Circle cx="40" cy="40" r="13" fill={cc2} />
      {[0,30,60,90,120,150,180,210,240,270,300,330].map((a,i)=>{ const r=8,rad=a*Math.PI/180; return <Circle key={`s${i}`} cx={40+r*Math.cos(rad)} cy={40+r*Math.sin(rad)} r="1.8" fill={c?'#6B3800':'#bbb'} />; })}
      {[15,45,75,105,135,165,195,225,255,285,315,345].map((a,i)=>{ const r=5,rad=a*Math.PI/180; return <Circle key={`s2${i}`} cx={40+r*Math.cos(rad)} cy={40+r*Math.sin(rad)} r="1.2" fill={c?'#5A2E00':'#aaa'} />; })}
    </Svg>
  );
}
...
function Flower2({ size = 40, colored }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      <Path d="M40 10 C28 10 16 22 18 36 C20 48 30 54 40 56 C50 54 60 48 62 36 C64 22 52 10 40 10Z" fill={c?'#C0392B':'#ddd'} />
      <Path d="M22 20 C14 28 14 42 22 50 C28 44 30 32 28 22Z" fill={c?'#E74C3C':'#ccc'} />
      <Path d="M58 20 C66 28 66 42 58 50 C52 44 50 32 52 22Z" fill={c?'#E74C3C':'#ccc'} />
      <Path d="M40 16 C32 16 24 26 26 38 C28 46 34 50 40 51 C46 50 52 46 54 38 C56 26 48 16 40 16Z" fill={c?'#E74C3C':'#ddd'} />
      <Path d="M28 24 C22 30 22 42 28 48 C32 44 34 34 32 26Z" fill={c?'#EC7063':'#ccc'} />
      <Path d="M52 24 C58 30 58 42 52 48 C48 44 46 34 48 26Z" fill={c?'#EC7063':'#ccc'} />
      <Path d="M40 22 C34 22 28 30 30 40 C32 46 36 49 40 49 C44 49 48 46 50 40 C52 30 46 22 40 22Z" fill={c?'#EC7063':'#ddd'} />
      <Path d="M40 28 C36 28 33 32 34 38 C35 42 38 44 40 44 C42 44 45 42 46 38 C47 32 44 28 40 28Z" fill={c?'#F1948A':'#eee'} />
    ...
    <Svg width={size} height={size} viewBox="0 0 80 80">
      {[0,72,144,216,288].map((a,i)=>(
        <G key={i} transform={`rotate(${a} 40 40)`}>
          <Path d="M40 12 C34 8 24 12 24 20 C24 28 32 32 40 38 C48 32 56 28 56 20 C56 12 46 8 40 12Z" fill={c?'#FFB7C5':'#e0e0e0'} />
          <Path d="M40 12 C37 10 34 12 34 16 C34 20 37 22 40 24" fill="none" stroke={c?'#FF8FA3':'#ccc'} strokeWidth="1" />
        </G>
      ))}
      <Circle cx="40" cy="40" r="8" fill={c?'#FFF0F3':'#f0f0f0'} />
      {[0,60,120,180,240,300].map((a,i)=>{ const rad=a*Math.PI/180; return <G key={`s${i}`}><Line x1="40" y1="40" x2={40+9*Math.cos(rad)} y2={40+9*Math.sin(rad)} stroke={c?'#FF69B4':'#ccc'} strokeWidth="1" /><Circle cx={40+10*Math.cos(rad)} cy={40+10*Math.sin(rad)} r="2" fill={c?'#FFD700':'#ddd'} /></G>; })}
...
function Flower4({ size = 40, colored }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      ...
      <Path d="M40 65 C30 58 22 50 26 44 C30 38 38 44 40 50" fill={c?'#3CB371':'#ccc'} />
      <Path d="M40 60 C50 53 58 45 54 39 C50 33 42 39 40 45" fill={c?'#2E8B57':'#bbb'} />
      <Path d="M24 44 C18 36 18 24 24 16 C28 10 34 12 36 20 C38 28 36 38 36 44Z" fill={c?'#E8001A':'#ddd'} />
      <Path d="M56 44 C62 36 62 24 56 16 C52 10 46 12 44 20 C42 28 44 38 44 44Z" fill={c?'#E8001A':'#ddd'} />
      <Path d="M32 46 C28 38 28 24 32 14 C34 8 38 8 40 14 C42 8 46 8 48 14 C52 24 52 38 48 46Z" fill={c?'#FF2233':'#eee'} />
...
function Flower5({ size = 40, colored }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      ...
      {[20,25,30,35,40,45,50].map((y,i)=>(<G key={i}><Ellipse cx={40+(i%2===0?-2:2)} cy={y} rx="4.5" ry="3" fill={c?'#7B68EE':'#ccc'} /><Ellipse cx={40+(i%2===0?-2:2)} cy={y-1} rx="3" ry="2" fill={c?'#9370DB':'#ddd'} /></G>))}
      {[[28,44],[52,39],[30,34],[50,30]].map(([x,y],i)=>(<G key={`s${i}`}><Ellipse cx={x} cy={y} rx="4" ry="2.5" fill={c?'#7B68EE':'#ccc'} /><Ellipse cx={x} cy={y-1} rx="2.5" ry="1.8" fill={c?'#9370DB':'#ddd'} /></G>))}
    </Svg>
  );
}
...
function Flower6({ size = 40, colored }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      {[0,18,36,54,72,90,108,126,144,162,180,198,216,234,252,270,288,306,324,342].map((a,i)=>(<Ellipse key={i} cx="40" cy="13" rx="3" ry="10" fill={c?'#FFFEF0':'#e8e8e8'} stroke={c?'#E8E0C0':'#d0d0d0'} strokeWidth="0.5" transform={`rotate(${a} 40 40)`} />))}
      {[9,27,45,63,81,99,117,135,153,171,189,207,225,243,261,279,297,315,333,351].map((a,i)=>(<Ellipse key={`b${i}`} cx="40" cy="18" rx="2" ry="7" fill={c?'#FFF8E0':'#e0e0e0'} transform={`rotate(${a} 40 40)`} opacity={0.8} />))}
      ...
      {[0,30,60,90,120,150,180,210,240,270,300,330].map((a,i)=>{ const rad=a*Math.PI/180; return <Circle key={`p${i}`} cx={40+7*Math.cos(rad)} cy={40+7*Math.sin(rad)} r="1.2" fill={c?'#A07800':'#999'} />; })}
    </Svg>
  );
}
...
function Flower7({ size = 40, colored }) {
  ...
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      {[0,60,120,180,240,300].map((a,i)=>(<Ellipse key={i} cx="40" cy="12" rx="6" ry="14" fill={c?'#FFFACD':'#e8e8e8'} stroke={c?'#E8E000':'#ccc'} strokeWidth="0.5" transform={`rotate(${a} 40 40)`} />))}
      ...
      {[0,40,80,120,160,200,240,280,320].map((a,i)=>{ const rad=a*Math.PI/180,r1=10,r2=13; return <Line key={i} x1={40+r1*Math.cos(rad)} y1={40+r1*Math.sin(rad)} x2={40+r2*Math.cos(rad)} y2={40+r2*Math.sin(rad)} stroke={c?'#FF4500':'#999'} strokeWidth="1.5" />; })}
      <Circle cx="40" cy="40" r="5" fill={c?'#FFD700':'#eee'} />
      {[0,72,144,216,288].map((a,i)=>{ const rad=a*Math.PI/180; return <G key={`st${i}`}><Line x1="40" y1="40" x2={40+6*Math.cos(rad)} y2={40+6*Math.sin(rad)} stroke={c?'#8B6914':'#ccc'} strokeWidth="1" /><Circle cx={40+7*Math.cos(rad)} cy={40+7*Math.sin(rad)} r="1.5" fill={c?'#DAA520':'#ddd'} /></G>; })}
    </Svg>
  );
}
...
function Flower8({ size = 40, colored }) {
  const c = colored;
  const cols = c ? ['#FF4444','#FF8C00','#FFD700','#4CAF50','#2196F3','#9C27B0','#FF69B4'] : new Array(7).fill('#ddd');
  return (
    <Svg width={size} height={size} viewBox="0 0 80 80">
      {cols.map((col,i)=>{ const a=i*(360/7); return (<G key={i} transform={`rotate(${a} 40 40)`}><Path d="M40 40 C34 32 28 20 34 10 C37 4 40 8 40 16 C40 8 43 4 46 10 C52 20 46 32 40 40Z" fill={col} opacity={c?0.92:1} /><Path d="M40 16 C38 12 36 10 36 14" stroke={c?'rgba(255,255,255,0.4)':'#e0e0e0'} strokeWidth="1" fill="none"/></G>); })}
      <Circle cx="40" cy="40" r="10" fill={c?'#FFFFFF':'#f0f0f0'} />
      <Circle cx="40" cy="40" r="7" fill={c?'#F8F8F8':'#e8e8e8'} />
      {cols.map((col,i)=>{ const rad=(i*(360/7))*Math.PI/180; return <Circle key={`c${i}`} cx={40+5*Math.cos(rad)} cy={40+5*Math.sin(rad)} r="2" fill={c?col:'#ccc'} />; })}
...
function PlantSVG({ stage, colored }) {
  ...
  if(stage===1) return(<Svg width={24} height={28} viewBox="0 0 28 32"><Ellipse cx="14" cy="28" rx="8" ry="3" fill="#8B6914" opacity={0.2}/><Rect x="12.5" y="14" width="3" height="14" rx="1.5" fill={stem}/><Ellipse cx="14" cy="12" rx="6" ry="6" fill={green1}/></Svg>);
  if(stage===2) return(<Svg width={29} height={40} viewBox="0 0 34 46"><Ellipse cx="17" cy="43" rx="10" ry="3" fill="#8B6914" opacity={0.18}/><Rect x="15.5" y="18" width="3" height="25" rx="1.5" fill={stem}/><Ellipse cx="17" cy="15" rx="7" ry="7" fill={green2}/></Svg>);
  if(stage===3) return(<Svg width={33} height={52} viewBox="0 0 38 58"><Ellipse cx="19" cy="55" rx="11" ry="3" fill="#8B6914" opacity={0.18}/><Rect x="17.5" y="24" width="3" height="31" rx="1.5" fill={stem}/><Path d="M19 50 C8 45 5 34 15 36 C20 37 19 50 19 50Z" fill={green3}/><Path d="M19 50 C30 45 33 34 23 36 C18 37 19 50 19 50Z" fill={green1}/><Ellipse cx="19" cy="20" rx="8" ry="8" fill={green2}/></Svg>);
  if(stage===4) return(<Svg width={37} height={62} viewBox="0 0 42 70"><Ellipse cx="21" cy="67" rx="12" ry="3" fill="#8B6914" opacity={0.18}/><Rect x="19.5" y="30" width="3" height="37" rx="1.5" fill={stem}/><Path d="M21 56 C10 51 8 41 17 42 C21 43 21 56 21 56Z" fill={green3}/><Path d="M21 56 C32 51 34 41 25 42 C21 43 21 56 21 56Z" fill={green1}/><Rect x="19" y="16" width="4" height="12" rx="2" fill={stem}/><Ellipse cx="21" cy="16" rx="6" ry="9" fill={floral}/></Svg>);
  return(<Svg width={42} height={72} viewBox="0 0 48 80"><Ellipse cx="24" cy="77" rx="13" ry="3" fill="#8B6914" opacity={0.18}/><Rect x="22.5" y="38" width="3" height="39" rx="1.5" fill={stem}/><Path d="M24 64 C12 59 10 48 20 49 C24 50 24 64 24 64Z" fill={green3}/><Path d="M24 64 C36 59 38 48 28 49 C24 50 24 64 24 64Z" fill={green1}/><Ellipse cx="24" cy="14" rx="5" ry="10" fill={c?'#ff9ab8':'#ddd'}/><Ellipse cx="24" cy="14" rx="5" ry="10" transform="rotate(60 24 24)" fill={c?'#ffb4cc':'#eee'}/><Ellipse cx="24" cy="14" rx="5" ry="10" transform="rotate(120 24 24)" fill={c?'#ff9ab8':'#ddd'}/><Ellipse cx="24" cy="14" rx="5" ry="10" transform="rotate(180 24 24)" fill={c?'#ffb4cc':'#eee'}/><Ellipse cx="24" cy="14" rx="5" ry="10" transform="rotate(240 24 24)" fill={c?'#ff9ab8':'#ddd'}/><Ellipse cx="24" cy="14" rx="5" ry="10" transform="rotate(300 24 24)" fill={c?'#ffb4cc':'#eee'}/><Circle cx="24" cy="24" r="8" fill={c?'#ffe066':'#eee'}/><Circle cx="24" cy="24" r="5" fill={c?'#ffd040':'#e0e0e0'}/><Circle cx="22" cy="23" r="1.2" fill="#7a4a10"/><Circle cx="26" cy="23" r="1.2" fill="#7a4a10"/><Path d="M21.5 25.5 Q24 28 26.5 25.5" stroke="#7a4a10" strokeWidth="1.2" fill="none" strokeLinecap="round"/></Svg>);
}
...
function UnlockAnimation({ flower, index, onClose }) {
  ...
  useEffect(() => {
    Animated.sequence([
      ...
      Animated.stagger(50, petalAnims.map(a => Animated.timing(a, { toValue:1, duration:400, useNativeDriver:true }))),
    ]).start();
  }, []);
  ...
  return (
    <View style={ua.overlay}>
      {petalAnims.map((a, i) => {
        ...
        return (<Animated.View key={i} style={[ua.petal,{left:x,transform:[{translateY}],opacity}]}><Svg width={14} height={14} viewBox="0 0 20 20"><Ellipse cx="10" cy="10" rx="5" ry="8" fill={['#FF8FA3','#FFB7C5','#FFD700','#9370DB','#7EC860'][i%5]} /></Svg></Animated.View>);
      })}
      <Animated.View style={[ua.card, { transform:[{scale:scaleAnim}], opacity:opacAnim }]}>
        ...
        <View style={ua.flowerBig}><FlowerComp size={80} colored /></View>
